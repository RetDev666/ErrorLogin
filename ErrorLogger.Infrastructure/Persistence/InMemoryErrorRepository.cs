...
namespace ErrorLogger.Infrastructure.Persistence
{
   public class InMemoryErrorRepository : IErrorRepository
   {
       ...
       {
           var filteredErrors = _errors.Values.Where(e => e.Status == status);
       ...
       {
           if (_errors.TryRemove(id, out _))
           {
               _logger.LogInformation("Видалено помилку з ID: {ErrorId}", id);
               return Task.FromResult(true);
           }

           _logger.LogWarning("Не вдалося видалити помилку ID {ErrorId}, помилка не знайдена", id);
           return Task.FromResult(false);
       }
   }
}